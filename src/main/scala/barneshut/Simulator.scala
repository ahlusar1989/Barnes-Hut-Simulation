...
//  Given an existing boundaries object and a body,
  // the updateBoundaries updates the minX, minY, maxX and maxY values so that the boundaries include the body.
...
//  During the Barnes-Hut simulation some of the bodies occasionally become distanced from the other bodies and
  // achieve velocity such that they will never again come into the proximity of the rest of the bodies.
  // We can safely eliminate such bodies from the simulation, as they are no longer relevant. The condition we will
  // use to detect such bodies will be that they are sufficiently far away from the center of mass of all the bodies,
  // and that their relative speed in the direction away from the center of mass is greater than the escape velocity.
